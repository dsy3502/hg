...
func (s *Server) initK8SConfigStore(args *PilotArgs) error {
	...
	if features.EnableGatewayAPI {
		...
		s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
			leaderelection.
				NewLeaderElection(args.Namespace, args.PodName, leaderelection.BuildClusterScopedLeaderElection(features.HigressGatewayStatusLeader), args.Revision, s.kubeClient).
				AddRunFunction(func(leaderStop <-chan struct{}) {
		...
		if features.EnableGatewayAPIDeploymentController {
			s.addTerminatingStartFunc(func(stop <-chan struct{}) error {
				leaderelection.
					NewLeaderElection(args.Namespace, args.PodName, leaderelection.BuildClusterScopedLeaderElection(features.GatewayDeploymentController), args.Revision, s.kubeClient).
					AddRunFunction(func(leaderStop <-chan struct{}) {
