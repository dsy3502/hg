...
func (c *Controller) handleStatusUpdates(configs []config.Config) {
	log.Errorf("start status update start")
	if c.status == nil || !c.statusEnabled.Load() {

		log.Errorf("start status update error")
		return
	}
	log.Errorf("start status update")

	for _, cfg := range configs {
		ws := cfg.Status.(*kstatus.WrappedStatus)
		log.Errorf("gatewayclass %s; status: %v; Dirty: %s", cfg.Name, ws.Status, ws.Dirty)
		if ws.Dirty {
